(function() {
  ...
  _.last = function(array, n) {
	...
	var indx = array.length - n; 
	if (n >= array.length) return array;
	return n === undefined ? array[lastIndex] : array.slice(indx);
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
	...
	if (accumulator === undefined) {
		...
		for (var o = 1; o < collection.length; o++) {
			accumulator = iterator(accumulator, collection[o]);
		}
	} else {
		for (var u = 0; u < collection.length; u++) {
			accumulator = iterator(accumulator, collection[u]);
		}
	}
  ...
  _.every = function(collection, iterator) {
	...
	_.each(collection,function(item) {
		if (iterator) {
			if (!iterator(item)) retval = false;
		} else
			if (!item) retval = false;
	});
	return retval;
  };
